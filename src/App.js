...
function TextItem(props) {
  return (
    <div>
      <p>Name: {props.names[props.index]}</p>
      <p>Age: {props.ages[props.index]}</p>
      <Button
        onClick={() =>
          props.updateDetails(props.names[props.index], props.age, props.index)
        }>
        Reset Details
      </Button>
    </div>
...
function App() {
  ...
  function updateDetails(name, age, index) {
    ...
    let namearray = names.filter((item) => item != currentName);
    let agearray = ages.filter((item) => item !== currentAge);
    namearray.push("");
    agearray.push("");
    setNames(namearray);
    setAges(agearray);
